...
export function RPN2InfixSlide(input){
    ...
    for (let i = 0; i < toEval.length; i++){
        ...
        // currentComponent should be an operator if we're here
            ...
            slideData.push({
                stackState : evalStack.slice(),
                expressionRemaining: toEval.slice(i + 1),
                slideText: `${currentComponent} is an operator, so we pop the top 2 elements, ${a} and ${b}, from the stack, and push ${a} ${currentComponent} ${b} to the stack`
            });
        }
    }
    ...
    slideData.push({
        ...
        expressionRemaining: "",
        slideText: `Done! We have converted ${input.join(" ")} to get ${evalStack[0]}.`
    });
